...
const Keyboard = {
     rowOne: [
          { keyCode: '192', name: 'Backquote', simbols: ['ё', 'Ё', '`', '~'], class: 'key' },
...
function init() {
     ...
     text.className = "text";
     text.innerHTML = 'Для смены языка воспользуйтесь сочетанием клавиш: Left Shift + Alt';
...
window.addEventListener("keydown", function (UI) {
     UI = UI || window.event;
     UI.preventDefault();
     let el;
     let key;
          ...
          default:
               ...
               for (el in Keyboard) {
                    for (let i = 0; i < Keyboard[el].length; i++) {
                         if (UI.keyCode == Keyboard[el][i]['keyCode']) {
     ...
     key = document.getElementsByClassName(UI.code);
...
window.addEventListener("keyup", function (UI) {
     UI = UI || window.event;
     UI.preventDefault();
     let el;
     let key;
     ...
     if (UI.key !== 'Shift' && pressed.has("Shift")) {
          pressed.clear();
          pressed.add('Shift');
     }
     else {
...
window.addEventListener('click', function (UI) {
     UI = UI || window.event;
     UI.preventDefault();

     let textarea = document.getElementById("input");

     if ((UI.target.className == 'case down') || (UI.target.className.indexOf('key') == 0 && UI.target.className.indexOf('keyboard') == -1)) {
          switch (UI.target.innerText) {
               case 'ENTER':
                    textarea.value += '\n';
                    break;
               case 'Backspace':
               ...
               case 'Tab':
                    break;
               default:
                    textarea.value += UI.target.innerText;
          }
     }
